...
The Footnotes module can be used to create automatically numbered az_wysiwyg
...
az_wysiwyg are placed at the end of the text. You can also use a
<code>[az_wysiwyg]</code> or <code>[az_wysiwyg /]</code> tag to position it
anywhere you want. 
...
Version 2.4 introduces an optional feature that identical az_wysiwyg are
...
then the "Configure" tab. By default, az_wysiwyg are separate unless you specify
...
version of az_wysiwyg.

<h3>INSTALL INSTRUCTIONS FOR FOOTNOTES.MODULE</h3>

1. Copy the contents of this module to sites/all/modules/az_wysiwyg folder.
2. Login as site administrator.
3. Activate az_wysiwyg on the administer->modules page.
4. To use the az_wysiwyg filter in some input formats, go to Configuration ->
   Text formats.
5. For the Text formats you want to support az_wysiwyg markup, select configure 
   and activate a suitable az_wysiwyg filter. 

<h3>TIPS n TRICKS (CSS)</h3>

Many Footnotes users don't want the az_wysiwyg to show in teasers (such as the
...
number/label in the list of az_wysiwyg. This class is now the LI element
...
html filter, and in particular you should have az_wysiwyg first and html filter
later.
...
Version 2.0 uses a new markup for the list of az_wysiwyg. It is an UL list with
...
The Views API functionality only works in the common use case when az_wysiwyg
are at the end of the text, but does not work correctly when using [az_wysiwyg]
...
The HTML az_wysiwyg were seriously enhanced by "beginner" (on drupal.org) and
